...
└── postgres/                   ← PostgreSQL 專屬
    ...
    ├── conventions.md
    ├── errors.md
    └── performance.md          ← 效能事件紀錄（前後 EXPLAIN + 量化）
```
...
必讀 `_global/rules.md`。其餘依當下主題按需讀取：Angular → `angular/conventions.md` + `stack.md`；Next.js → `nextjs/conventions.md` + `stack.md`；Python → `python/conventions.md` + `stack.md`；Electron → `electron/conventions.md` + `stack.md`；PostgreSQL → `postgres/conventions.md` + `stack.md`（+ `performance.md` / `errors.md` 視情況）；TypeScript → `_shared/typescript.md`；Tailwind → `_shared/tailwind.md`；專案根目錄有 `openspec/` → `_shared/spectra.md`；遇錯先查 `[framework]/errors.md`；涉及 Claude Code 工具/skill/subagent/slash command 行為 → `_global/skill.md`。不需每次全讀。
...
當 Hsin 表示對話結束（「好了」「謝謝」「先這樣」「我去吃飯了」），或主要任務完成，**主動提議**整理進筆記，可更新：`session-log.md`（一定加，本次摘要）、`errors.md`（有踩坑解法）、`conventions.md`（有新決議）、`stack.md`（有版本變更）、`performance.md`（PG 有效能優化事件）。

同意後直接修改檔案，最後列出改了哪些，提示 `git diff` 檢視，並建議 commit message。**不要等 Hsin 說「請更新」才動作**。
...
### `performance.md`（事件型，目前僅 `postgres/`）
記具體效能優化案例的前後對比。模板（**「適用版本」必填**）：

```markdown
### 主題（例：訂單列表頁載入 2s → 80ms）
- **適用版本**：PostgreSQL 14-16
- **日期**：YYYY-MM-DD
...
與 `errors.md` 差別：errors 記「壞了 → 修好」、performance 記「能跑但慢 → 變快」。「通用優化原則」放 `conventions.md`，本檔記**事件**。
