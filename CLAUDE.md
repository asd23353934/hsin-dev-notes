...
│   └── decisions.md            ← ADR：技術選型決策紀錄（為什麼選 X 不選 Y）
│
├── angular/                    ← Angular 專屬
│   ├── stack.md                ← 套件版本（事實）
...
### `_shared/decisions.md`（ADR / 選型決策型）
記「為什麼選 X 不選 Y」的高階技術選型，含被否決的替代方案與失效條件。模板見該檔內部。
**與 `conventions.md` 差別**：ADR 是一次性決定（會失效但不常改），conventions 是持續遵循的規則。
